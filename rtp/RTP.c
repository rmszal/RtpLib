...
int			RTP_Create(context *the_context)
{
	...
	conx_cc = (conx_context_t *) pvPortMalloc(sizeof(conx_context_t));
...
int			RTP_Destroy(context cid)
{
	...
	conx_cc = (conx_context_t *) pvPortMalloc(sizeof(conx_context_t));
	buff = (remote_address_t *) pvPortMalloc(sizeof(remote_address_t));
	buff2 = (remote_address_t *) pvPortMalloc(sizeof(remote_address_t));
...
int			RTP_Add_Send_Addr(context cid, char *addr, u_int16 port, u_int8 ttl)
{
	...
	RTP_Session_Restore_Cxinfo(cid, (void **)&conx_cc);
	remote_dev_data = (remote_address_t *) pvPortMalloc(sizeof(remote_address_t));
	...
	if (ismulticast)
	{
		...
		#endif
		#ifdef BARE_ARM
			Set_TTL_BareArm(remote_dev_data->socket, remote_dev_data->ttl);
		#endif
	}
...
int			RTP_Rem_Send_Addr(context cid, char *addr, u_int16 port, u_int8 ttl)
{
	...
	RTP_Session_Restore_Cxinfo(cid, (void **)&conx_cc);
	buffer1 = (remote_address_t *) pvPortMalloc(sizeof(remote_address_t));
	buffer2 = (remote_address_t *) pvPortMalloc(sizeof(remote_address_t));
...
int			Add_Extension(context cid, u_int32 extension)
{
	...
	else
	{
		...
		context_list[cid]->hdr_extension->hd_ext = pvPortRealloc(context_list[cid]->hdr_extension->hd_ext, sizeof(u_int32) * nb_ext);
...
{
	vPortFree(context_list[cid]->hdr_extension);
	return (0);
}
...
int			Add_CRSC(context cid, u_int32 csrc)
{
	...
	else
	{
		...
		context_list[cid]->CSRCList = pvPortRealloc(context_list[cid]->CSRCList, sizeof(u_int32) * context_list[cid]->CSRClen);
...
int			Init_Context(context context_num, u_int32 nb_context)
{
	...
	{
		tab_ssrc = pvPortRealloc(tab_ssrc, sizeof(u_int32) * nb_context);
...
int			RTP_Context_Create(context *cid)
{
	...
	else if (context_list == NULL)
	{
		...
		context_list = (sess_context_t**) pvPortCalloc(MAX_CONTEXT, sizeof(sess_context_t*));
	}
	else
	{
		i = above_context;
		above_context += 10;
		context_list = pvPortRealloc(context_list, sizeof(sess_context_t*) *above_context);
	}
	context_list[i] = pvPortMalloc(sizeof(sess_context_t));
	nb_context++;
...
int			RTP_Context_destroy(context cid)
{
	...
	if (context_list[cid]->CSRCList)
		vPortFree(context_list[cid]->CSRCList);
	if (context_list[cid]->hdr_extension)
		vPortFree(context_list[cid]->hdr_extension);
	vPortFree(context_list[cid]);
...
int			RTP_Sd_Pkt(context cid, u_int32 ts, u_int8 marker, u_int16 pti, u_int8 *payload, int len, u_int8 padd_len)
{
	...
	sd_buffer  = (u_int8 *) pvPortMalloc(hd_len + len + padd_len);
...
rtp_pkt			*Get_RTP_Hdr(char *msg, int  sz)
{
  ...
  u_int16		hdr_sz;
  u_int8		len_padding;
  rtp_hdr		*rtp_hdr_msg = 0;
  rtp_ext		*rtp_ext_msg = 0;
  rtp_pkt		*rtp_pkt_msg = 0;
...
void			Free_Tmp_Mem(rtp_pkt *pkt)
{
   ...
   * Free Memoire. 
   */
	vPortFree(pkt->RTP_extension->hd_ext);
	vPortFree(pkt->RTP_extension);
	vPortFree(pkt->RTP_header->csrc);
	vPortFree(pkt->RTP_header);
	vPortFree(pkt);
  return;
}
